...
中间件采用直接投递模型。微信仍以低噪声投递为默认；微信 2.4.4 进度实验通过 route 级 `/progress` 模式控制普通文本进度和结构化工具生命周期：
...
- 微信渠道通过 `ChannelDeliveryPolicy` 声明不投递 task-start，但允许 `/progress brief|detailed|tools|silent`；默认 `silent` 不发送普通文本进度和结构化工具生命周期。
...
3. 确认当前微信 progress 模式：
   ...
   /status
   ```
   2.4.4 实验分支默认应显示为低噪声的 `/progress silent`；先确认最终回复稳定，再手动切到 `/progress tools` 观察结构化工具生命周期。
