...
- 微信通过 delivery policy 不发送 task-start，但允许 `/progress brief|detailed|tools|silent` 控制普通文本进度和结构化工具生命周期；默认 `silent` 不发送进度和工具生命周期。
- 对高频 delta 输出做合并，不逐字刷屏。
- 普通用户默认只看关键阶段和最终结果。
- 管理员或 debug 模式可以看到更细的事件，例如命令开始、命令输出摘要、文件变更摘要。
- 对同一 turn 的阶段性回复要能归并到一个会话上下文。
- `codex exec --json` 模式下阶段性回复以可见事件为准，非微信渠道至少包含简短开始处理提示、reasoning summary、命令/工具/文件变更进度和最终回复；微信默认只发送最终回复、Plan mode 最终计划、错误、审批、队列提示、媒体发送结果、主动命令回复和结构化工具生命周期。
...
当前主干微信渠道不展示上述中间阶段，只保留关键投递；用户可发送 `/fff` 触发一次静默入站刷新。2.4.4 实验中，`/progress detailed` 和 Plan turn 的临时 detailed effective mode 可以展示命令/工具输出摘要，`/progress tools` 只展示结构化工具生命周期。
